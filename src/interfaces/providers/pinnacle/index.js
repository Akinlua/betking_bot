...
    this.dumpFilePath = path.join(
      process.cwd(),
      "data/logs",
      this.dumpFilePathName,
    );
    this.retryCount = 0;
    this.maxRetries = 5;
...
    console.log(
      chalk.green(
        `[Provider] Polling Started [INTERVAL-${this.config.interval}]`,
      ),
    );
...
  async poll() {
    if (this.state.status === this.constructor.Status.STOPPED) {
        return;
    }
    ...
    try {
        ...
        if (notifications && notifications.length > 0) {
            ...
            if (this.config.storeData) {
                try {
                    ...
                    console.error(
                        "[Provider] Error writing data to dump file:",
                        writeError,
                    );
                }
            }
        ...
        this.state.status = this.constructor.Status.POLLING;
        console.log(
            `[Provider] Status: ${this.state.status} | ${this.state.message}`,
        );
        this.scheduleNextPoll();

    } catch (error) {
        console.error("[Provider] Polling error:", error.message);
        ...
        const delay = this.config.interval * 1000 * Math.pow(2, this.retryCount);
        console.warn(
            chalk.yellow(
                `[Provider] Retrying in ${delay / 1000}s... (Attempt ${this.retryCount + 1}/${this.maxRetries})`,
            ),
        );
        this.retryCount++;
...
scheduleNextPoll(delay = this.config.interval * 1000) {
...
stopPolling() {
    ...
    this.state.message = "Polling has been stopped by user or due to a fatal error.";
...
        throw new Error(
          "All items in odds array must be numbers greater than 1.",
        );
      }
...
            console.warn(
              "[devigOdds] Additive method resulted in a non-positive probability. Falling back to multiplicative.",
            );
            trueProbs = impliedProbs.map((p) => p / totalImpliedProb);
          }
          break;
...
            for (let i = 0; i < 100; i++) {
              let k = (low + high) / 2;
              let sum = impliedProbs.reduce(
                (acc, p) => acc + Math.pow(p, k),
                0,
              );
